Feature: Capacity Analytics
  # This feature contains the sanity test for forecast insight generation of the snort memory metric
  ...
  Scenario: Test forecast insight generation for Snort Memory metric
    Then backfill metrics for a suitable device over 2160 hour(s)
      ...
      | mem                    | mem=used_percentage_snort                 | 15.25       | 60.72     | 0                  | 129600                 | 24                       | 9.25      |
    Then trigger capacity analytics workflow
    Then verify if an MEMORY_SNORT_THRESHOLD_FORECAST_BREACH insight with state ACTIVE is created with a timeout of 60 minute(s)
